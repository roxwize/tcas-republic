/**
 * @typedef {{uname:string, positions?:{pos:number,period:number}[], medals?:{pos:number,period:number}[], sockpuppet_of?:string}} Member
 */
...
const positions = require("./views/_data/positions.json");

function get_election_year() {
  const now = new Date(Date.now());
  if (now.getUTCMonth() >= 5 && now.getUTCDay() >= 7)
    return now.getUTCFullYear();
  else return now.getUTCFullYear() - 1;
}
/** @param {string} uname */
const url_friendly = (uname) =>
  uname.toLowerCase().replace(/[^A-Za-z0-9]/g, "");
...
module.exports = function (eleventyConfig) {
  ...
  eleventyConfig.addPassthroughCopy({
    ...
    "famfamfam-silk/dist/sprite/famfamfam-silk.png":
      "static/css/famfamfam-silk.png"
  });
    ...
    (member, tcas = true) => {
      ...
      if (member.positions) {
        for (let pos of member.positions) {
          if (pos.period === y) {
            if (pos.pos === 0)
              out += '<div class="famfamfam-silk bullet_star"></div>';
            if (pos.pos === 1)
              out += '<div class="famfamfam-silk bullet_yellow"></div>';
          }
        }
      }
      ...
      <a href="${
        tcas
          ? `https://twocansandstring.com/users/${url_friendly(member.uname)}`
          : `/members/${member.uname.replaceAll(" ", "")}/`
      }">${member.uname}</a>\
    `;

      if (member.medals) {
        for (let medal of member.medals) {
          out += `<span class="emoji">${
            positions.medals[medal.pos].icon
          }</span>`;
        }
      }
